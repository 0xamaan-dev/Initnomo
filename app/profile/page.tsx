...
export default function ProfilePage() {
    ...
    useEffect(() => {
        if (username) {
            setNewUsername(username.replace('.initnomo', ''));
        }
    }, [username]);

    const handleUpdateUsername = async () => {
        if (!address || !newUsername) return;
        const formattedUsername = `${newUsername.toLowerCase()}.initnomo`;
    ...
    if (!isConnected) {
        return (
            ...
            >
                ...
                <h1 className="text-2xl font-black text-white uppercase tracking-widest mb-4">INITNOMO Registry</h1>
    ...
    const normalizedReferralCode = referralCode ? referralCode.replace(/^bynomo-/i, 'initnomo-') : '';
            ...
            >
                <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Link href="/rewards" className="px-5 py-3 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-gray-200 transition-all active:scale-95 shadow-xl">
                            Referral System
                        </Link>
                            ...
                            >
                                <div className="space-y-5">
                                    ...
                                    <div className="p-4 bg-white/[0.03] border border-white/10 rounded-xl flex items-center justify-between">
                                        <span className="font-mono font-black text-lg tracking-widest text-white/80">{normalizedReferralCode || '---'}</span>
                                        <button onClick={() => copyToClipboard(normalizedReferralCode || '', 'Code')} className="text-white/20 hover:text-white transition-all">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => {
                                            if (!normalizedReferralCode?.trim()) {
                                                toast.error('Referral code not ready yet');
                                                return;
                                            }
                                            openReferralShareOnX(referralLandingUrl(normalizedReferralCode));
                                        }}
