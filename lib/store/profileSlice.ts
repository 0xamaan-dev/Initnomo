...
export const createProfileSlice: StateCreator<ProfileState> = (set, get) => ({
    username: (typeof window !== 'undefined' && localStorage.getItem('initnomo_username')) || null,
    accessCode: (typeof window !== 'undefined' && localStorage.getItem('initnomo_access_code')) || null,
        ...
        try {
            ...
            if (data) {
                ...
                if (typeof window !== 'undefined') {
                    if (data.username) localStorage.setItem('initnomo_username', data.username);
                    if (data.access_code) localStorage.setItem('initnomo_access_code', data.access_code);
                }
            }
        } catch (error) {
