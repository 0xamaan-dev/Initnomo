...
export const createBalanceSlice: StateCreator<BalanceState> = (set, get) => ({
  ...
  accountType: (typeof window !== 'undefined' && localStorage.getItem('initnomo_account_type') as 'real' | 'demo') || 'real', // Persist account type
  userTier: 'free',
  isLoading: false,
  error: null,
  ...
  fetchBalance: async (address: string) => {
    ...
    const currency = resolveHouseLedgerCurrency({
      network: (get() as any).network || 'INIT',
  ...
  toggleAccountType: () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('initnomo_account_type', newType);
    }
  },
