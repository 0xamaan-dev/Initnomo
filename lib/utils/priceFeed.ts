 ...
 * Supports: BTC, SUI, SOL, AND CUSTOM TOKENS (INITNOMO)
 */
...
export const CUSTOM_TOKENS = {
  INITNOMO: 'Faw8wwB6MnyAm9xG3qeXgN1isk9agXBoaRZX9Ma8BAGS'
} as const;
...
const PRICE_CACHE_STORAGE_KEY = 'initnomo_last_price_cache_v1';
...
export class PythPriceFeed {
  ...
  async fetchPrice(): Promise<PriceData> {
    try {
      if (this.asset === 'INITNOMO') {
        const response = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${CUSTOM_TOKENS.INITNOMO}`, {
        ...
        throw new Error('No pairs found for INITNOMO');
      }
  ...
  static async fetchAllPrices(): Promise<Record<string, number>> {
    ...
    try {
      ...
      // 2. INITNOMO (DexScreener)
      try {
        const initnomoRes = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${CUSTOM_TOKENS.INITNOMO}`, {
          signal: AbortSignal.timeout(5000) // 5s timeout
        });
        if (initnomoRes.ok) {
          const data = await initnomoRes.json();
          if (data.pairs && data.pairs.length > 0) {
            results.INITNOMO = parseFloat(data.pairs[0].priceUsd);
          }
        }
      } catch (tokenErr) {
...
export class MockPriceFeed {
  ...
  constructor(asset: AssetType = 'BTC', basePrice?: number, volatility: number = 0.001, trend: number = 0) {
    this.asset = asset;
    const defaults: Record<string, number> = { BTC: 50000, BNB: 600, INITNOMO: 0.1 };
