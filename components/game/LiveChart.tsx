...
const ASSET_CONFIG: Record<AssetType, { name: string; symbol: string; pair: string; logo: string; category: 'Crypto' | 'Metals' | 'Forex' | 'Stocks' | 'Commodities'; decimals: number }> = {
  ...
  // Custom
  INITNOMO: { name: 'Initnomo Token', symbol: 'INITNOMO', pair: 'INITNOMO/SOL', logo: 'https://ucarecdn.com/7a6858e8-6e54-473c-b6df-7f938d28a38c/-/preview/256x256/', category: 'Crypto', decimals: 6 },
...
export const LiveChart: React.FC<LiveChartProps> = ({ betAmount, setBetAmount }) => {
  ...
  const filteredAssets = useMemo(() => {
    return (Object.keys(ASSET_CONFIG) as AssetType[]).filter(assetId => {
      // Push-only rollout: hide the INITNOMO token from the asset picker for now.
      if (assetId === 'INITNOMO') return false;
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('initnomo_watchlist_assets');
      if (raw) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('initnomo_watchlist_assets', JSON.stringify(watchlist));
    } catch (e) {
